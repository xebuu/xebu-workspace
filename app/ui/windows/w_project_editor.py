...
from app.models.project_models import CopierItem, LinkItem, ProcessDef, ScriptItem
...
class ProjectEditorWindow(QDialog):
    ...
    def __init__(self, existing: Optional[ProcessDef] = None, parent=None):
        ...
        head.addWidget(QLabel("Nombre:"))
        head.addWidget(self.edt_name)
    ...
    def _open_add_chooser(self):
        ...
        row.addWidget(btn_script)
        row.addWidget(btn_link)
        row.addWidget(btn_copier)
        lay.addLayout(row)
        btns = QDialogButtonBox(QDialogButtonBox.Close)
        lay.addWidget(btns)
        dlg.setStyleSheet(
            "QDialog{background:#1b2224;} QLabel{color:#EFEDE1;} QPushButton{padding:6px 10px;}"
        )
    ...
    def _browse_file_into(self, line_edit: QLineEdit, filter_text: str = "Todos (*.*)"):
        path, _ = QFileDialog.getOpenFileName(
            self, "Elegir archivo", str(Path.home()), filter_text
        )
        if path:
